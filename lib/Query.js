...
function makeKey(options) {
	...
	return _crypto2.default.createHash('sha1').update(_circularJson2.default.stringify(cacheObj, jsonReplacer)).digest('hex');
};
...
function checkBodyFormat(req) {
	if (req.contentType() !== 'application/json') return new errors.BadRequestError("content-type must be application/json");
	if (req.body == undefined) return new errors.MissingParameterError("no body");
	...
	return function (req, res, next) {
		...
		return parseBody(options.parse, req.body).then(() => {
			...
			return options.model.sequelize.transaction(t => {
				...
				for (let rowIdx = 0, len = req.body.length; rowIdx < len; rowIdx++) {
					// execute upsert
					p = p.then(options.model.insertOrUpdate.bind(options.model, req.body[rowIdx], Object.assign({ transaction: t }, options))).then(data => {
						results.push(data);
					}).catch(err => {
						throw { row: rowIdx, message: err.name, errors: err.errors, fields: err.fields };
					});
				}

				// format results
				return p.then(formatResults.bind(this, options.format, results)).then(rslt => {
					results = rslt;
				})
				// invalidate cache
	...
	return function (req, res, next) {
		...
		return parseBody(options.parse, req.body).then(() => {
			...
			return options.model.sequelize.transaction(t => {
				...
				for (let rowIdx = 0, len = req.body.length; rowIdx < len; rowIdx++) {
					// execute insert
					p = p.then(options.model.create.bind(options.model, req.body[rowIdx], Object.assign({ transaction: t }, options))).then(data => {
						results.push({ row: rowIdx, updated: data });
					}).catch(err => {
						throw { row: rowIdx, message: err.name, errors: err.errors, fields: err.fields };
					});
				}
				...
				// format results
				return p.then(formatResults.bind(this, options.format)).then(rslt => {
					results = rslt;
				})
				// invalidate cache
				.then(invalidateCache.bind(this, options))
	...
	return function (req, res, next) {
		...
		return parseBody(options.parse, req.body).then(() => {
			...
			return options.model.sequelize.transaction(t => {
				...
				for (let rowIdx = 0, len = req.body.length; rowIdx < len; rowIdx++) {
					// calculate where clause
					p = p.then(calcWhere.bind(this, options, req.body[rowIdx])).then(where => {
						// delete records
						return options.model.destroy(where ? Object.assign({}, qry, { where: where }) : qry);
					}).then(data => {
						results.push({ row: rowIdx, deleted: data });
					}).catch(err => {
						throw { row: rowIdx, message: err.name, errors: err.errors, fields: err.fields };
					});
				}
				...
				// invalidate cache
				return p.then(invalidateCache.bind(this, options)).then(rslt => {
					// send result
					res.send(results);
					return next();
				});
			...
			return options.model.sequelize.transaction(t => {
				...
				for (let rowIdx = 0, len = req.body.length; rowIdx < len; rowIdx++) {
					...
					p = p.then(calcWhere.bind(this, options, req.body[rowIdx])).then(where => {
						...
						results.push({ row: rowIdx, updated: data[0] });
					}).catch(err => {
						throw { row: rowIdx, message: err.name, errors: err.errors, fields: err.fields };
					});
				}
				...
				if (cached != undefined) {
					...
					if (qry.transaction === true) {
						return qry.sequelize.transaction(t => {
							qry.transaction = t;
							// execute raw query
							return qry.sequelize.query(qry.sql, qry).then(finish);
						});
					} else {
						// execute raw query
						return qry.sequelize.query(qry.sql, qry).then(formatResults.bind(this, options.format)).then(finish);
					}
				}
			});
		}).catch(err => next(formatBadRequestError(err)));
		...
		return parseBody(options.parse, req.body).then(() => {
			...
			return options.sequelize.transaction(t => {
				...
				for (let rowIdx = 0, len = req.body.length; rowIdx < len; rowIdx++) {
					...
					p = p.then(baseQry.replacements.bind(this, req.body[rowIdx])).then(replacements => {
						if (replacements) qry.replacements = replacements;else delete qry.replacements;

						// calculate binds
						return qry.bind(req.body[rowIdx]);
					}).then(bind => {
						if (bind) qry.bind = bind;else delete qry.bind;
						...
						return qry.sequelize.query(qry.sql, qry).then(formatResults.bind(this, options.format)).then(setCache.bind(this, options, shard, key));
					}).then(data => {
						results.push({ row: rowIdx, result: data });
					}).catch(err => {
						console.log(err);throw { row: rowIdx, message: err.name, errors: err.errors, fields: err.fields };
					});
				}
